...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (cursorFollower && !isTouchDevice() && window.innerWidth > 480) {
        ...
        const interactiveElements = document.querySelectorAll('a, button, .carousel-controls .dot, .carousel-item');
    ...
    if (flower) {
        ...
        function animateFlower(flower) {
            ...
            const timingFactor = window.innerWidth <= 480 ? 0.7 : 
                               (window.innerWidth <= 809 ? 0.85 : 1);
        ...
        function createParticles(container) {
            ...
            const particleCount = window.innerWidth <= 480 ? 5 : 
                               (window.innerWidth <= 809 ? 10 : 15);
            ...
            const intervalRate = window.innerWidth <= 480 ? 1200 : 
                              (window.innerWidth <= 809 ? 1000 : 800);
        ...
        function createParticle(container) {
            ...
            const scaleFactor = Math.min(containerRect.width, containerRect.height) / 500;  // Scale factor based on container size relative to original 500px
            ...
            const sizeScale = window.innerWidth <= 480 ? 0.7 : (window.innerWidth <= 809 ? 0.85 : 1);
            ...
            particle.style.backgroundColor = `hsla(${210 + hue}, 70%, 80%, ${opacity})`;
    ...
    if (hamburgerIcon && mainNav) {
        ...
        document.addEventListener('click', function(event) {
            if (!mainNav.contains(event.target) && !hamburgerIcon.contains(event.target) && mainNav.classList.contains('open')) {
                hamburgerIcon.classList.remove('open');
                mainNav.classList.remove('open');
            }
        });
